...
class MEMS_CTRNN_ACC:
    ...
    def calc_params(self):
        ...
        self.states2 = np.full(self.size, 0.0, dtype=float)
    ...
    def print_model_abstract(self):
        ...
        for i in range(self.size):
            ...
            s1 += str(round(self.states[i], 9)) + ', '
            s2 += str(round(self.states[i], 9)) + ', '
            for j in range(self.size):
    ...
    def set_circuit_size(self, new_size):
        self.size = new_size
        self.states1 = np.full(new_size, 0.0, dtype=float)
    ...
    def euler_step(self, step_size=None, use_dim_equation=False,
        ...
        for i in range(self.size):
            ...
            self.states2[i] += self.step_size * \
                (-self.states2[i] - self.mem_K3 * (self.states[i] ** 3) -
                 k1 * self.states[i] + mem_theta -
                 self.mem_win * (v_mem ** 2) /
