...
<<echo=FALSE, message=FALSE, warning=FALSE, fig.width=4, fig.asp=0.9>>=
final_sum <- final_data %>%
        group_by(obj_type_name, valid) %>%
        summarise(val =n())

final_sum$percentage <- paste(round(100 * final_sum$val/nrow(final_data), 2) , "%", sep="")
ggplot(final_sum, aes(factor(obj_type_name), val, fill = valid)) +
          geom_bar(stat = "identity") +
          geom_text(aes(label = percentage, y = nrow(final_data)/2 - val /2 ),  color = "black") + theme_light()+
  #scale_fill_manual(values = c("red", "steelblue"), guide = FALSE)
  #scale_fill_brewer(palette='Dark2') +
  xlab('Object Type')+ ylab('Scenario count')+
  guides(fill=guide_legend(title="Outcome"))


ggplot(final_data,aes(x = X1,y=X2, color = valid)) +
  geom_point() +
  #scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
   xlab('X Position of Object')+ ylab('X Position of Agent')+
  guides(fill=guide_legend(title="Outcome"))+
facet_grid(obj_type_name ~.) + theme_light() + theme(legend.position='none')
