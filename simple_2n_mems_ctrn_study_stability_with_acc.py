...
from mems_ctrnn_acc import MEMS_CTRNN_ACC
...
def gen_input(t):
    i1 = 50 if t > 0.0020 else 0
    i2 = 50 if t > 0.00400 else 0
...
if __name__ == "__main__":
    ...
    with open('debug.csv', 'w') as fi:
        ...
        for time in np.arange(0, run_duration, step_size):
            ...
            for t in range(int(d)):
                c2.set_neuron_external_input(0, i1)
                c2.set_neuron_external_input(1, i2)
                c2.euler_step(1/d)

                out1_1.append((time + t * step_size/d, c2.states[0]))
                out2_1.append((time + t * step_size/d, c2.states[1]))

                out1_2.append((time + t * step_size/d, c2.states2[0]))
                out2_2.append((time + t * step_size/d, c2.states2[1]))
                print(time + t * step_size/d)
